...
void gfx::setupZaxis(TH2 &h2, bool log){
   ...
   if (!palAx){
      std::cerr<<TString::Format("no z axis found for '%s'",h2.GetName());
      return;
   }
   // thinner:
   ...
   , pL_("padL"+TString::Itoa(s_count,10),"padL",0,0,1,1-fraction)
{
   ...
   pU_.SetRightMargin(0.03);
   pU_.Draw();
   pU_.Update();
   pL_.SetTopMargin(0);
   pL_.SetRightMargin(0.03);
...
void gfx::SplitCan::draw_lumi(bool simulation, bool drawLumiText)
{
   TString cmsText     = "CMS";
   ...
   // float H = pad.GetWh();
   // float W = pad.GetWw();
   float l = pU_.GetLeftMargin();
   float t = pU_.GetTopMargin();
   float r = pU_.GetRightMargin();
