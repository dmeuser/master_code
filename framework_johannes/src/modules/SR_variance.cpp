...
static io::RootFileSaver saver("plots.root","SR_variance");
...
io::Logger SR_Vg_effect(("SR_Vg_shape_variance.tex"));
io::Logger SR_gJ_effect(("SR_gJ_shape_variance.tex"));
io::Logger mu_SR_Vg_graph(("mu_SR_Vg_graph.tex"));
io::Logger mu_SR_gJ_graph(("mu_SR_gJ_graph.tex"));
io::Logger pdf_SR_Vg_graph(("pdf_SR_Vg_graph.tex"));
io::Logger pdf_SR_gJ_graph(("pdf_SR_gJ_graph.tex"));
...
std::map<int,float> getVgScale()
{
   ...
   path += "/output/weights_fitresult.tex";
...
//   std::vector<TString> vsGJComponents{"GJets_DR"};
   ...
   if (weight_index < 9) {
      ...
      pdf_SR_Vg_graph << Vg_1;
      pdf_SR_Vg_graph << Vg_2;
      pdf_SR_Vg_graph << Vg_3;
      pdf_SR_Vg_graph << Vg_4;
      
      pdf_SR_gJ_graph << gJ_1;
      pdf_SR_gJ_graph << gJ_2;
      pdf_SR_gJ_graph << gJ_3;
      pdf_SR_gJ_graph << gJ_4;

      bin1_Vg[weight_index]=VarVg1;
      bin2_Vg[weight_index]=VarVg2;
      bin3_Vg[weight_index]=VarVg3;
      bin4_Vg[weight_index]=VarVg4;

      bin1_gJ[weight_index]=VargJ1;
      bin2_gJ[weight_index]=VargJ2;
      bin3_gJ[weight_index]=VargJ3;
      bin4_gJ[weight_index]=VargJ4;      
   }
...
void run()
{
   for (int i = 0; i < nWeights; i++){ 
      plot("pre_ph165/c_MET300/MT300/STg600/","STg_"+std::to_string(i),{600,800,1000,1300,1600},{200,200,300,300},SR,i);
   }
   ...
   //in function binX array is only filled from weight 9 on, so it is only PDF
   for (int i = 0; i < nPDF; i++){
      mean_Vg_1+=bin1_Vg[i]/nPDF;
      mean_Vg_2+=bin2_Vg[i]/nPDF;
      mean_Vg_3+=bin3_Vg[i]/nPDF;
      mean_Vg_4+=bin4_Vg[i]/nPDF;
      mean_gJ_1+=bin1_gJ[i]/nPDF;
      mean_gJ_2+=bin2_gJ[i]/nPDF;
      mean_gJ_3+=bin3_gJ[i]/nPDF;
      mean_gJ_4+=bin4_gJ[i]/nPDF;
   }

   float bin1_Vg_variance = ComputeStdDev(nPDF,bin1_Vg,mean_Vg_1);
   float bin2_Vg_variance = ComputeStdDev(nPDF,bin2_Vg,mean_Vg_2);
   float bin3_Vg_variance = ComputeStdDev(nPDF,bin3_Vg,mean_Vg_3);
   float bin4_Vg_variance = ComputeStdDev(nPDF,bin4_Vg,mean_Vg_4);
   float bin1_gJ_variance = ComputeStdDev(nPDF,bin1_gJ,mean_gJ_1);
   float bin2_gJ_variance = ComputeStdDev(nPDF,bin2_gJ,mean_gJ_2);
   float bin3_gJ_variance = ComputeStdDev(nPDF,bin3_gJ,mean_gJ_3);
   float bin4_gJ_variance = ComputeStdDev(nPDF,bin4_gJ,mean_gJ_4);

   std::cout << "relative bin1_Vg_variance: " << bin1_Vg_variance/nominalVg1 << std::endl;
   std::cout << "relative bin2_Vg_variance: " << bin2_Vg_variance/nominalVg2 << std::endl;
   std::cout << "relative bin3_Vg_variance: " << bin3_Vg_variance/nominalVg3 << std::endl;
   std::cout << "relative bin4_Vg_variance: " << bin4_Vg_variance/nominalVg4 << std::endl;
   std::cout << "relative bin1_gJ_variance: " << bin1_gJ_variance/nominalgJ1 << std::endl;
   std::cout << "relative bin2_gJ_variance: " << bin2_gJ_variance/nominalgJ2 << std::endl;
   std::cout << "relative bin3_gJ_variance: " << bin3_gJ_variance/nominalgJ3 << std::endl;
   std::cout << "relative bin4_gJ_variance: " << bin4_gJ_variance/nominalgJ4 << std::endl;
