            ...
            if isinstance(h, ROOT.TH1):
                if self.xsecs[i]:
                    ...
                    if type(self.xsecs[i]) == str:
                        ...
                        mass2 =int(name.split("/")[0].split("_")[1])
                        if self.names[0].find("GGM")!=-1:
                            xsec = aux.getXsecInfoSMS(mass, self.xsecs[i], mass2)[0]
                        else:
...
tching = Dataset("SMS-TChiNG_BF50N50G", "data/xSec_SMS_TChiNg_13TeV.pkl")
